...
| 8 | **Budget exhausted** | Kernel reservation at lease (amendment A2) — or settlement at outcome for lagged/federated usage — hits a zero budget dimension | `grant.exhausted` journaled; affected invocations → `budget-exceeded` (interrupted class, spine §3 object 3); spawning refused; running children of the grant subtree stopped (Claude Code's tree-wide enforcement contract — FACT, research/notes/anthropic-claude-code-agent-sdk.md) | Escalation up the grant lineage (A2A AUTH_REQUIRED chaining generalized — FACT/INFERENCE HIGH, research/notes/a2a-protocol.md): parent or human issues an attenuated top-up grant and resumes, or cancels | Grant (budgets as attenuated quantitative rights) + interrupted state + escalation chain |
